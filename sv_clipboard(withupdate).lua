...
local function CreateEntityFromTable(EntTable, Player)
	...
	if (not GENERIC) then
		...
		if (EntTable.ModelScale) then
			EntTable.ModelScale = nil
		end
...
local function AdvDupe2_Spawn()
	...
	if (Queue.Entity) then
		...
		if (Queue.Current > #Queue.SortedEntities) then
			local NEnts = #Queue.SortedEntities -- #Queue.CreatedEntities or 0
			
			local C1 = 0
			local C2 = 0
			
			local co = coroutine.create( function( )
				local NMax = NEnts - (NEnts - (math.floor(NEnts/3)*3))
				local val = 0
				local count = 0
				
				for k, Ent in pairs(Queue.CreatedEntities) do
					count = count + 1
					...
					if count >= val or count >= NMax then
						-- print(val, count)
						val = val + 3
			...
			local function cofunc( )
				local bool, A = coroutine.resume( co )
				if A then
					local P = math.Round( A / ( NEnts / 100 ) )
					
					if P > 100 then
						AdvDupe2.UpdateProgressBar(Queue.Player, 100)
					else
						AdvDupe2.UpdateProgressBar(Queue.Player, P)
					end
					-- print(A, NEnts)
					
					if Another_ReadyToWork then
						timer.Simple( 0.1, cofunc )
						C1 = C1 + 1
					else
						timer.Simple( 0.5, cofunc )
						C2 = C2 + 1
					end
				else
					if C1 > 0 and C2 > 0 then
						print("DUPLICATOR: C1: ", C1, " - C2: ", C2)
						print("DUPLICATOR: Lag --> ", math.Round( (C2*100)/(C1 + C2)), "%" )
					end
			...
			-- if NEnts then
				AdvDupe2.InitProgressBar(Queue.Player, "EntityMods:")
...
local function ErrorCatchSpawning()
	...
	while ticktotal >= 1 do
		...
		if (not status) then
			...
			undo.another_AddEntsRemove( Queue.CreatedEntities )
